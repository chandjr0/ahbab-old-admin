...
const UpdateExpense = () => {
  ...
  useState(() => {
    const fetchData = async () => {
      try {
        ...
        const [headData, expenseData] = await Promise.all([
          axios.get("/expense/head-fetch-all"),
          axios.get("/expense/single-fetch/" + expenseId),
        ]);
        setExpenseTypeOptions(headData?.data?.data);
        let data = expenseData?.data?.data;
  ...
  const formSubmitHandler = async (event) => {
    ...
    try {
      ...
      const res = await axios.patch(`/expense/update/${expenseData?._id}`, obj);
      if (res?.data?.success) {
  ...
  return (
    <div className="m-sm-30">
      <div className="mb-sm-30">
        <Breadcrumb routeSegments={[{ name: "Update Expense" }]} />
      </div>

      <Grid container>
        <Grid item md={6} xs={12}>
          <Card elevation={3}>
            <CardHeader title="Update Expense" />
