...
const CourierList = ({ dataList, setDataList, setUpdateData }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsLoading(true);
        let res = await axios.get("/courier/fetch-all");
  ...
  const deleteHandler = async () => {
    try {
      let res = await axios.delete(`/courier/delete/${deleteId}`);
