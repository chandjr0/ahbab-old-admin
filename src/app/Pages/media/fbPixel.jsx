...
const FbPixel = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        let res = await axios.get("/setting/admin/fetch-fb-script");
        if (res) {
  ...
  const formSubmitHandler = async (event) => {
    ...
    try {
      setIsLoading(true);
      const res = await axios.patch(`/setting/admin/update-fb-script`, data);
      if (res?.data?.success) {
  ...
  return (
    <div className="m-sm-30">
      <div className="mb-sm-30">
        <Breadcrumb routeSegments={[{ name: "Facebook Scripts" }]} />
      </div>

      <Grid container>
        <Grid item md={6} xs={12} className="mb-4">
          <Card elevation={3}>
            <CardHeader title="Add New Facebook Scripts" />
                  ...
                  <Grid item xs={12}>
                    <Box sx={{ mb: 2 }}>
                      ...
                      <TextField
                        ...
                        // minRows={10}
                        label=""
                        placeholder="paste access token code here..."
                        variant="outlined"
                        fullWidth
                        value={accessToken}
