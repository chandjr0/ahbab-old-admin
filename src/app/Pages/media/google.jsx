...
const FbPixel = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        let res = await axios.get("/setting/admin/fetch-google-script");
        if (res) {
          
          let data = res?.data?.data;
          setPixelScriptH(data?.header);
          setPixelScriptF(data?.body);
          setTagManager(data?.tagManager)
  ...
  const formSubmitHandler = async (event) => {
    ...
    try {
      setIsLoading(true);
      const res = await axios.patch(`/setting/admin/update-google-script`, {
        header: '',
        body: '',
  ...
  return (
    <div className="m-sm-30">
      <div className="mb-sm-30">
        <Breadcrumb routeSegments={[{ name: "Google Scripts" }]} />
      </div>

      <Grid container>
        <Grid item md={6} xs={12} className="mb-4">
          <Card elevation={3}>
            <CardHeader title="Add New Google Scripts" />
            ...
            {!isPageLoading ? (
              <form className="px-4 py-6" onSubmit={formSubmitHandler}>
                  ...
                  <Grid item xs={12}>
                    <Box sx={{ mb: 2 }}>
                      ...
                      <TextField
                        ...
                        minRows={10}
                        label=""
                        placeholder="paste your scripts code here..."
                        variant="outlined"
                        fullWidth
                        value={pixelScriptH}
                ...
                <Grid container spacing={3}>
                  <Grid item xs={12}>
                    <Box sx={{ mb: 2 }}>
                      <InputLabel className="mb-2 text-black">{`Tag Manager (ID)`}</InputLabel>
                      <TextField
                        label=""
                        ...
                        fullWidth
                        value={tagManager}
                        onChange={(e) => setTagManager(e.target.value)}
                      />
                    </Box>
                  </Grid>
                </Grid>
