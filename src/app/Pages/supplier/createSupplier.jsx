...
const CreateSupplier = () => {
  ...
  const formSubmitHandler = async (data) => {
    try {
      ...
      setIsLoading(true);
      const res = await axios.post(`/supplier/create`, obj);
      if (res?.data?.success) {
        reset();
        setIsDisabled(false);
        setFileList([]);
  ...
  return (
    <div className="m-sm-30">
      <div className="mb-sm-30">
        <Breadcrumb routeSegments={[{ name: "New Supplier" }]} />
      </div>

      <Grid container>
        <Grid item md={6} xs={12}>
          <Card elevation={3}>
            <CardHeader title="Add New Supplier" />
            ...
            <form className="px-4 py-6" onSubmit={handleSubmit(formSubmitHandler)}>
              ...
              <Button
                ...
                disabled={isLoading}
              >
                {isLoading ? <CircularProgress size={24} color="inherit" /> : "Add Supplier"}
              </Button>
            </form>
          </Card>
        </Grid>
