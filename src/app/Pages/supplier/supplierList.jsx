...
const SupplierList = () => {
  ...
  const deleteHandler = async () => {
    try {
      let res = await axios.delete(`/supplier/delete/${deleteId}`);
  ...
  const statusUpdateHandler = async (data) => {
    try {
      const createRes = await axios.patch(`/supplier/update/${data._id}`, {
        name: data?.name,
        phone: data?.phone,
      ...
      if (createRes?.data?.success) {
        openNotificationWithIcon("supplier status updated", "success");
      }
      setIsLoading(false);
    } catch (error) {
      openNotificationWithIcon(error?.response?.data?.message, "error");
                      ...
                      <TableBody>
                        {dataList.map((data, index) => (
                          <TableRow key={index}>
                            ...
                            <Tooltip title={"phone"} placement="bottom-start">
                              <TableCell className="capitalize" align="left">
                                {data?.phone}
                              </TableCell>
                            </Tooltip>
                            ...
                            <TableCell>
                              <Tooltip title={"edit"} placement="bottom-start">
                                <IconButton
                                  onClick={() => {
                                    history.push(`update-supplier/${data._id}`);
                                  }}
                                  style={{
