...
const CartDetails = ({
                  ...
                  cartProducts.map((prod) =>
                    prod?.uid === value?.uid
                      ? {
                          ...
                          quantity: Number(prod?.quantity) + 1,
                          subTotal:
                            (Number(prod?.quantity) + 1) * Number(prod?.price),
                        }
                      : prod
                  )
                );
    ...
    },
    {
      ...
      format: (value) => (
        <IconButton
          ...
          onClick={() => {
            setCartProducts(cartProducts.filter((cart) => cart?.uid !== value.uid));
            if (value.isCombo) {
              setCombos(
                combos.filter(cart => cart?.uid !== value.uid)
              );
            }
          }}
        >
  ...
  useEffect(() => {
    ...
    setRows(dataArray);
    // console.log("dataArray: ", cartProducts);
  }, [cartProducts]);
