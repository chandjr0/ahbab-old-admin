...
const PrintComponent = React.forwardRef((props, ref) => {
  ...
  const [steadfastClientID, setSteafastClientId] = useState("");
    ...
    const fetchData = async () => {
      try {
        ...
        if (res) {
          ...
          setSteafastClientId(data?.steadfast?.STEADFAST_CLIENT_ID);
        }
      } catch (err) {
        console.log(err);
      }
    ...
    <div ref={ref} className="print-content">
      ...
      {orderData.length > 0 &&
        orderData?.map((data, index) => {
          // Group two orders per page
          if (index % 2 === 0) {
            return (
              <div key={index} className="">
                <div className="">
                  {/* First order */}
                  {renderOrder(data, settingsData, steadfastClientID, couriers)}

                  {/* Second order (if exists) */}
                  {orderData[index + 1] &&
                    renderOrder(
                      orderData[index + 1],
                      settingsData,
...
const renderOrder = (data, settingsData, steadfastClientID, couriers) => {
            ...
            <div className="text-left">
              <div className="my-2">
                ...
                >
                  ...
                  <span style={{ color: "#000" }}>
                    {data?.deliveryAddress?.city?.city_name},
      ...
      const fetchData = async () => {
        try {
          ...
          const [res, settingD] = await Promise.all([
            axios.post(`/reseller-order/admin/multiple-orders`, {
