...
export default function OrderList() {
  ...
  useEffect(() => {
    let fetchData = async () => {
      try {
        ...
        if (searchValue !== "") {
          res = await axios.get(
            `/reseller-order/admin/search-for-paid/${searchValue}`,
            {
              value: searchValue,
            }
          );
  ...
  useEffect(() => {
    let localData = localStorage.getItem('orders')? JSON.parse(localStorage.getItem('orders')):[]
  ...
  const submitPayment = async () => {
    ...
    try {
      let res = await axios.post(`/reseller-order/admin/update-order-paid`, {
        orderIds: ids,
      });
  ...
  return (
    <div className="m-sm-30">
      ...
      <Card className="border-radius-0 ">
        <CardHeader  title="SEARCH ORDERS TO PAY" />
            ...
            {dataList?.length > 0 ? (
              <Box className="ml-3">
                <Button
                  ...
                  style={{ background: "green", color: "#fff",minWidth:'150px' }}
                >
                  PAID
                </Button>
              </Box>
