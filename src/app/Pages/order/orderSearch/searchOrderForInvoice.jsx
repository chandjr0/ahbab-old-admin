...
export default function OrderList() {
  ...
  let fetchData = async (val) => {
    ...
    if (val.length >= 10) {
      try {
        let res = null;
        setIsLoading(true);
        res = await axios.get(`/admin-order/search-for-invoice/${val}`);
  ...
  const submitPayment = async () => {
    ...
    try {
      let res = await axios.post(`/admin-order/update-orders-invoice`, {
        orderIds: ids,
      });
  ...
  const downloadCSVHandler = () => {
    ...
    writeFile(wb, `${moment().format("L").replace(/\//g, "-")}-Ready-to-ship-list.xlsx`);
  };

  return (
    <div className="m-sm-30">
      <Card className="border-radius-0 ">
        <CardHeader title="READY TO SHIP" />
